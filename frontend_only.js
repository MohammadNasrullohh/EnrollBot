...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && req.url === "/play_audio") {
    ...
    req.on("end", async () => {
      ...
      try {
        await streamAudio(latestTelemetry.ip, vol, file);
  ...
  if (req.method === "POST" && req.url === "/test_max") {
    ...
    req.on("end", async () => {
      ...
      try {
        await streamTestTone(latestTelemetry.ip, vol);
  ...
  ws.on('message', (message, isBinary) => {
    try {
      if (isBinary) {
        ...
        if (buffer.length >= 4 && buffer.subarray(0, 4).toString() === "MIC:") {
          const session = voiceSessions.get(ws);
          if (session && !session.processing) {
            session.chunks.push(buffer.subarray(4));
            session.bytes += buffer.length - 4;
          }
        }
        return;
      }
      ...
      if (text.startsWith("VOICE:START")) {
        ...
        latestTelemetry = JSON.parse(text.replace(/:\s*(nan|NaN)/gi, ':null'));
