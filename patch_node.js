...
// We know there are two <canvas id="drawCanvas" ...> wait!
// The second one uses: "const c=\drawCanvas,ctx=c.getContext('2d',{willReadFrequently:true});"

let index = code.indexOf("const c=drawCanvas,ctx=c.getContext('2d',{willReadFrequently:true});");
if (index === -1) {
    console.log("Could not find the target string.");
    process.exit(1);
}

// Find the end of this block which is the end of sync() and clearDraw assignment:
// "enterDraw.onclick=sync;clearDraw.onclick=()=>{ctx.fillStyle='#000';ctx.fillRect(0,0,240,320);sync();fetch('/cmd/W',{method:'POST'})};"
let endIndex = code.indexOf("fetch('/cmd/W',{method:'POST'})};", index);
if (endIndex === -1) {
    console.log("Could not find end index");
    process.exit(1);
}
endIndex += "fetch('/cmd/W',{method:'POST'})};".length;
...
code = code.substring(0, index) + new_js + code.substring(endIndex);

fs.writeFileSync('web_serial_server.js', code);
console.log("Success");
