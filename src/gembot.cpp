...
const float AI_OUTPUT_GAIN = 0.75f;
      ...
      i2s_write(I2S_NUM_0, buffer, sizeof(buffer), &bytes_written, portMAX_DELAY);
       ...
       if (data != NULL) {
           ...
           for (int i=0; i<samples; i++) {
               ...
               if (aiSpeaking) outputSample = (int32_t)(outputSample * AI_OUTPUT_GAIN);
  ...
  if (type == WStype_CONNECTED) {
    ...
    if (text.startsWith("AUDIO:")) {
      ...
      if (length == 9606 && memcmp(payload, "FRAME:", 6) == 0) {
          ...
          while (millis() - startWait < 2000UL) {
              sent = xRingbufferSend(audioRingBuf, payload, length, pdMS_TO_TICKS(5));
              if (sent == pdTRUE) break;
              
              // Keep UI responsive while waiting for ringbuffer space
              unsigned long now = millis();
...
void updateMPU() {
  ...
  float tiltX = -ax;
  float tiltY = ay;
  if (fabs(tiltX) > 0.65f) { // Increased from 0.45 to prevent random curious
    if (sustainedTiltStartMs == 0) sustainedTiltStartMs = now;
    else if (now - sustainedTiltStartMs > 1000) {  // Increased from 700
        curiousUntilMs = now + 900;
        reactWithExpression(tiltX > 0 ? 31 : 26, 1100);
    }
  } else {
...
void setDfVolume(int volume) {
  ...
  if (dfPlayerReady) myDFPlayer.volume(dfVolume);
}
  ...
  if (now < manualExpressionUntilMs && currentExpressionId >= 0) {
    ...
    if (friendlyAge < 1300UL) {
      ...
      if (now >= nextIdleEmotionMs) {
        const int idleChoices[] = {1, 13, 24, 26, 28, 31, 50, 51};
...
void setup() {
  ...
  // Setup MPU6050
  Wire.begin(21, 22);
  Wire.setTimeOut(150); // Prevent WDT crash if MPU wiring glitches
  if (!setupRawMPU()) {
  ...
  // Test: draw directly on display to verify TFT works
  display.fillScreen(TFT_BLACK);
...
void sendTelemetry() {
  ...
  if (now - lastTelemetryMs < 500UL) return;
  lastTelemetryMs = now;
