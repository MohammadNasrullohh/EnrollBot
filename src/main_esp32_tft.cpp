...
// Temporary pins for standard ESP32, though user wants just screen for now
#define MAX_BCLK_PIN 32
#define MAX_LRC_PIN 33
#define MAX_DIN_PIN 35
#define MIC_SD_PIN -1
...
void fillEllipseLocal(int16_t x0, int16_t y0, int16_t rx, int16_t ry, uint16_t color) {
  ...
  for (x = 0, y = ry, sigma = 2*b2+a2*(1-2*ry); b2*x <= a2*y; x++) {
    spr.drawLine(x0 - x, y0 + y, x0 + x, y0 + y, color);
    spr.drawLine(x0 - x, y0 - y, x0 + x, y0 - y, color);
    if (sigma >= 0) {
      sigma += fa2 * (1 - y);
      y--;
...
void drawBrow(int16_t x, int16_t y, int16_t w, int16_t angleX10, int dx, int dy) {
  ...
  for (int16_t i = 0; i < w; i++) {
    ...
    spr.drawPixel(bx, by,     OLED_WHITE);
    spr.drawPixel(bx, by + 1, OLED_WHITE);
  }
}
...
void drawEyeNormal(int16_t cx, int16_t cy, int16_t rx, int16_t ry,
  ...
  if (blink) {
    for (int16_t x = cx - rx; x <= cx + rx; x++) {
      spr.drawPixel(x, cy,     OLED_WHITE);
      spr.drawPixel(x, cy + 1, OLED_WHITE);
    }
    return;
  }
  ...
  for (int16_t y = pcy - ps; y <= pcy + ps; y++) {
    for (int16_t x = pcx - ps; x <= pcx + ps; x++) {
      ...
      if (fdx * fdx + fdy * fdy <= 1.0f)
        spr.drawPixel(x, y, OLED_BLACK);
    }
  }
  spr.drawPixel(pcx - ps / 3, pcy - ps / 3, OLED_WHITE);
  if (lid > 0) {
...
void drawHeart(int16_t cx, int16_t cy, int16_t sz, int dx, int dy) {
  ...
  for (int16_t y = -sz; y <= sz; y++) {
    for (int16_t x = -sz; x <= sz; x++) {
      ...
      if (v <= 0.0f) spr.drawPixel(cx + x, cy + y - sz/10, OLED_WHITE);
    }
  }
}
...
void drawStar(int16_t cx, int16_t cy, int16_t sz, int dx, int dy) {
  ...
  for (uint8_t i = 0; i < 10; i++) {
    ...
    spr.drawLine(x0, y0, x1, y1, OLED_WHITE);
  }
}

void drawXEyes(int16_t cx, int16_t cy, int16_t sz, int dx, int dy) {
  cx += dx; cy += dy;
  spr.drawLine(cx - sz, cy - sz, cx + sz, cy + sz, OLED_WHITE);
  spr.drawLine(cx - sz, cy + sz, cx + sz, cy - sz, OLED_WHITE);
  spr.drawLine(cx - sz + 1, cy - sz, cx + sz + 1, cy + sz, OLED_WHITE);
  spr.drawLine(cx - sz + 1, cy + sz, cx + sz + 1, cy - sz, OLED_WHITE);
}
...
void drawSquint(int16_t cx, int16_t cy, int16_t rx, int16_t ry, int dx, int dy) {
  ...
  for (int16_t y = cy - ry; y <= cy - ry / 2; y++) {
    for (int16_t x = cx - rx; x <= cx + rx; x++) {
      float fdx = (float)(x - cx) / rx;
      float fdy = (float)(y - cy) / ry;
      if (fdx*fdx + fdy*fdy <= 1.0f) spr.drawPixel(x, y, OLED_WHITE);
    }
  }
  for (int16_t x = cx - rx; x <= cx + rx; x++) spr.drawPixel(x, cy + ry / 3, OLED_WHITE);
}

void drawWink(int16_t cx, int16_t cy, int dx, int dy) {
  cx += dx; cy += dy;
  spr.drawLine(cx - 11, cy + 1, cx + 11, cy + 1, OLED_WHITE);
  spr.drawLine(cx - 11, cy + 2, cx + 11, cy + 2, OLED_WHITE);
}

void drawDollarSign(int16_t cx, int16_t cy, int16_t sz, int dx, int dy) {
  cx += dx; cy += dy;
  spr.drawCircle(cx, cy, sz / 2, OLED_WHITE);
  spr.drawLine(cx, cy - sz, cx, cy + sz, OLED_WHITE);
  spr.drawLine(cx + 1, cy - sz, cx + 1, cy + sz, OLED_WHITE);
}
...
void drawSpiral(int16_t cx, int16_t cy, int16_t sz, int dx, int dy) {
  ...
  for (float a = 0; a < TWO_PI * 3; a += 0.2f) {
    ...
    spr.drawLine(prev_x, prev_y, nx, ny, OLED_WHITE);
    prev_x = nx; prev_y = ny;
  }
}
...
void drawLightning(int16_t cx, int16_t cy, int16_t sz, int dx, int dy) {
  cx += dx; cy += dy;
  spr.drawLine(cx,        cy - sz,    cx + sz/2, cy,       OLED_WHITE);
  spr.drawLine(cx + sz/2, cy,         cx,        cy + sz/4, OLED_WHITE);
  spr.drawLine(cx,        cy + sz/4,  cx + sz/2, cy + sz,  OLED_WHITE);
}
...
void drawMouth(MouthParam m, int dx, int dy) {
  ...
  switch (m.type) {
    case 0: // line
      spr.drawLine(x1, y1, x2, y1, OLED_WHITE);
      spr.drawLine(x1, y1 + 1, x2, y1 + 1, OLED_WHITE);
      break;
    case 1: // smile
    case 2: { // frown
      ...
      for (uint8_t i = 1; i <= 20; i++) {
        ...
        spr.drawLine(prevX, prevY, bx, by, OLED_WHITE);
        prevX = bx; prevY = by;
      }
      break;
    }
    ...
    case 4: // wavy
      for (int16_t x = x1; x <= x2; x++) {
        ...
        spr.drawPixel(x, wy, OLED_WHITE);
        spr.drawPixel(x, wy + 1, OLED_WHITE);
      }
      break;
    case 5: { // smirk
    ...
    case 6: { // teeth
      ...
      for (uint8_t i = 0; i < 4; i++) {
        ...
        spr.drawLine(tx, y1, tx, y1 + m.dep / 2, OLED_BLACK);
      }
      break;
    }
    case 7: // scream
    ...
    case 8: { // zigzag
      ...
      for (int16_t i = 0; i < steps; i++) {
        ...
        spr.drawLine(nx0, ny0, nx1, ny1, OLED_WHITE);
      }
      break;
    }
  }
...
void drawTear(int16_t x, int16_t y, int dx, int dy) {
  ...
  for (int16_t i = 0; i < 8; i++) {
    spr.drawPixel(x, y + i, OLED_WHITE);
    if (i > 4) spr.drawPixel(x - 1, y + i, OLED_WHITE);
  }
}
...
void drawBitmapScaled(const unsigned char* bits, int srcW, int srcH,
  ...
  int SCALE = 2;
  int OFFSET_X = (320 - (128 * SCALE)) / 2;
  int OFFSET_Y = (240 - (64 * SCALE)) / 2;
  ...
  for (int yy = 0; yy < dstH; yy++) {
    ...
    for (int xx = 0; xx < dstW; xx++) {
      ...
      if (px < 0 || px >= SCREEN_WIDTH) {
        if (startX != -1) { spr.drawFastHLine(startX, py, px - startX, OLED_WHITE); startX = -1; }
        continue;
      }
      int sx = (xx * srcW) / dstW;
      if (bitmapBitOn(bits, srcW, sx, sy)) {
        if (startX == -1) startX = px;
      } else {
        if (startX != -1) { spr.drawFastHLine(startX, py, px - startX, OLED_WHITE); startX = -1; }
      }
    }
    if (startX != -1) spr.drawFastHLine(startX, py, (x + dstW) - startX, OLED_WHITE);
  }
}
...
void drawBitmapPupil(const BitmapLayerSpec& eye, int dx, int dy,
  ...
  spr.fillCircle(px, py, radius, OLED_BLACK);
  if (radius >= 3) spr.drawPixel(px - 1, py - 1, OLED_WHITE);
}

void drawMiniHeart(int x, int y) {
  spr.fillCircle(x - 1, y, 1, OLED_WHITE);
  spr.fillCircle(x + 1, y, 1, OLED_WHITE);
  spr.drawPixel(x, y + 2, OLED_WHITE);
}

void drawSoftSparkle(int x, int y, int r) {
  spr.drawPixel(x, y, OLED_WHITE);
  spr.drawFastHLine(x - r, y, r * 2 + 1, OLED_WHITE);
  spr.drawFastVLine(x, y - r, r * 2 + 1, OLED_WHITE);
}
...
void drawBitmapFaceStable(BitmapMpuFace face, unsigned long now, bool active,
  ...
  if (face == BMP_FACE_NORMAL) {
    ...
    mouthExtraY += 1;
    for (int i = 0; i < 3; i++) {
      spr.drawLine(20 + i * 4, 46 + i % 2, 23 + i * 4, 45 + i % 2, OLED_WHITE);
      spr.drawLine(103 + i * 4, 45 + i % 2, 106 + i * 4, 46 + i % 2, OLED_WHITE);
    }
    ...
    if (laughPhase > 0.6f) {
      spr.drawPixel(22, 38, OLED_WHITE);
      spr.drawPixel(106, 38, OLED_WHITE);
    }
    ...
    mouthExtraY += 1;
    spr.drawPixel(36, 48 + (int)(soft * 1.0f), OLED_WHITE);
    spr.drawPixel(92, 48 - (int)(soft * 1.0f), OLED_WHITE);
    ...
    if (thoughtPhase == 1) {
      ...
      spr.drawPixel(105, dotY, OLED_WHITE);
      spr.drawPixel(110, dotY + 2, OLED_WHITE);
    ...
    spr.drawLine(29, 29 + browLift, 54, 27 + browLift, OLED_WHITE);
    spr.drawLine(76, 27 - browLift, 101, 29 - browLift, OLED_WHITE);
  } else if (face == BMP_FACE_SAD) {
    lookY += 1;
    eyeExtraY += (int)(fabsf(sinf(t * 1.5f)) * 1.0f);
    mouthExtraY += (int)(sinf(t * 1.7f) * 1.0f);
    spr.drawLine(29, 14, 51, 18, OLED_WHITE);
    spr.drawLine(78, 18, 100, 14, OLED_WHITE);
    if (actionPulse > 0.45f) {
      drawBitmapScaled(img_tear, 6, 10, 25, 42 + (int)(actionPulse * 5.0f), 6, 10);
    }
    ...
    drawBitmapScaled(img_sweat, 4, 8, 104, 13 + (int)(fabsf(tremble) * 2.0f), 4, 8);
    ...
    for (int i = 0; i < 3; i++) {
      ...
      spr.drawCircle(px, py, 1, OLED_WHITE);
    }
    spr.drawPixel(110 + (int)(sinf(roll) * 4.0f), 10 + (int)(cosf(roll) * 3.0f), OLED_WHITE);
    ...
    mouthExtraY += 1;
    for (int i = 0; i < 4; i++) {
      spr.drawLine(18 + i * 3, 46 + (i & 1), 20 + i * 3, 45 + (i & 1), OLED_WHITE);
      spr.drawLine(103 + i * 3, 45 + (i & 1), 105 + i * 3, 46 + (i & 1), OLED_WHITE);
    }
    ...
    spr.drawLine(26, 26, 55, 24, OLED_WHITE);
    spr.drawLine(75, 24, 104, 26, OLED_WHITE);
    ...
    if (((now / 1400UL) % 4UL) == 2UL) {
      spr.drawPixel(109, 18, OLED_WHITE);
      spr.drawPixel(113, 16, OLED_WHITE);
      spr.drawPixel(116, 14, OLED_WHITE);
    }
    ...
    spr.drawLine(22, 16, 33, 7, OLED_WHITE);
    spr.drawLine(33, 7, 44, 16, OLED_WHITE);
    ...
    if (((now / 1100UL) % 3UL) == 1UL) {
      spr.drawPixel(107, 18, OLED_WHITE);
      spr.drawPixel(111, 16, OLED_WHITE);
    }
    ...
    spr.drawLine(28, 24, 54, 22 + (int)(inspect * 1.0f), OLED_WHITE);
    ...
    spr.drawLine(26, 24, 56, 21, OLED_WHITE);
    spr.drawLine(74, 21, 104, 24, OLED_WHITE);
    ...
    if (((now / 500UL) % 3UL) == 1UL) drawBitmapScaled(img_sweat, 4, 8, 104, 18, 4, 8);
    ...
    dy += 1;
    mouthExtraX -= 1;
    spr.drawLine(29, 17, 51, 20, OLED_WHITE);
    spr.drawLine(78, 20, 100, 17, OLED_WHITE);
    ...
    spr.drawLine(27, 19, 54, 23, OLED_WHITE);
    spr.drawLine(76, 23, 103, 19, OLED_WHITE);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      spr.drawCircle(px, py, 1 + (i & 1), OLED_WHITE);
    }
    spr.drawCircle(106 + (int)(sinf(orbit) * (3.0f + dizzyPower * 2.0f)),
                       13 + (int)(cosf(orbit) * (2.0f + dizzyPower * 2.0f)), 3, OLED_WHITE);
    spr.drawPixel(64 + (int)(cosf(orbit * 0.55f) * 14.0f), 8 + (int)(sinf(orbit * 0.55f) * 3.0f), OLED_WHITE);
    ...
    spr.fillCircle(sx, 18, 2, OLED_WHITE);
    spr.fillTriangle(sx - 2, 17, sx + 2, 17, sx, 10, OLED_WHITE);
  } else if (face == BMP_FACE_COLD) {
    float tremble = sinf(t * 5.2f) * 1.2f;
    dx += (int)tremble;
    eyeAddW -= 1;
    mouthExtraX += (int)(-tremble);
    spr.drawLine(18, 17, 22, 13, OLED_WHITE);
    spr.drawLine(22, 13, 26, 17, OLED_WHITE);
    spr.drawLine(106, 17, 110, 13, OLED_WHITE);
    spr.drawLine(110, 13, 114, 17, OLED_WHITE);
    ...
    spr.drawCircle(17, nY + 8, 2, OLED_WHITE);
    spr.drawFastVLine(20, nY, 8, OLED_WHITE);
    spr.drawFastHLine(20, nY, 5, OLED_WHITE);
    spr.drawCircle(109, nY + 11, 2, OLED_WHITE);
    spr.drawFastVLine(112, nY + 3, 8, OLED_WHITE);
    ...
    mouthExtraY += 1;
    spr.drawLine(25, 15, 54, 23, OLED_WHITE);
    spr.drawLine(76, 23, 105, 15, OLED_WHITE);
    spr.drawLine(26, 16, 54, 24, OLED_WHITE);
    spr.drawLine(76, 24, 104, 16, OLED_WHITE);
    if (superAngryMode) {
      drawBitmapScaled(img_angry_vein, 8, 8, 14, 7, 8, 8);
      spr.drawLine(108, 10, 121, 7, OLED_WHITE);
      spr.drawLine(108, 15, 122, 15, OLED_WHITE);
    }
  } else if (face == BMP_FACE_CRYING) {
    lookY += 2;
    dy += (int)(fabsf(sinf(t * 1.8f)) * 1.0f);
    mouthExtraY += (int)(sinf(t * 2.2f) * 1.0f);
    spr.drawLine(29, 15, 51, 19, OLED_WHITE);
    spr.drawLine(78, 19, 100, 15, OLED_WHITE);
  }
  ...
  if (face == BMP_FACE_CRYING) {
    ...
    spr.drawLine(37 + dx, 42 + dy, 36 + dx, 51 + dy + tearWave / 3, OLED_WHITE);
    spr.drawLine(93 + dx, 42 + dy, 94 + dx, 51 + dy + tearWave / 3, OLED_WHITE);
    spr.fillCircle(36 + dx, 53 + dy + tearWave / 3, 2, OLED_WHITE);
    spr.fillCircle(94 + dx, 53 + dy + tearWave / 3, 2, OLED_WHITE);
  }
...
void drawMochi(bool active) {
  ...
  if (!active && motion < 0.1f && now >= nextIdleExprMs) {
    ...
    if (false && random(0, 3) == 0 && !isWinking) { // User requested no blink
      isWinking = true;
  ...
  bool isBlinkingNow = false; // User requested no blink
...
void drawPingpong() {
  ...
  if (gamePhase == GAME_IDLE) {
    spr.setTextSize(1);
    spr.setTextColor(OLED_WHITE);
    spr.setCursor(40, 10);
    spr.print("PINGPONG");
    spr.setCursor(25, 28);
    spr.print("Hold: Keluar");
    spr.setCursor(25, 40);
    spr.print("Tap: Mulai");
    pushScaledSprite();
    return;
  ...
  if (gamePhase == GAME_OVER) {
    ...
    spr.setTextColor(OLED_WHITE);
    spr.setCursor(30, 20);
    if (scorePlayer >= 5) spr.print("KAMU MENANG!");
    else spr.print("KAMU KALAH!");
    spr.setCursor(20, 40);
    spr.print("Tap untuk Main");
    pushScaledSprite();
    return;
  }
  ...
  spr.setTextColor(OLED_WHITE);
  spr.setCursor(30, 4);
  spr.print(scorePlayer);
  spr.setCursor(85, 4);
  spr.print(scoreAi);

  // Draw Ball
  if (gamePhase == GAME_PLAYING) {
    spr.fillRect((int)ballX - 1, (int)ballY - 1, 3, 3, OLED_WHITE);
  ...
  // Draw Paddles
  // Player (Left)
  spr.fillRect(8, (int)paddlePlayerY - 8, 3, 16, OLED_WHITE);
  // AI (Right)
  spr.fillRect(117, (int)paddleAiY - 8, 3, 16, OLED_WHITE);
...
void drawMenu(const char* title, const char** options, int numOptions, int cursor) {
  ...
  spr.setCursor(0, 0);
  spr.print(title);
  spr.drawLine(0, 10, SCREEN_WIDTH, 10, OLED_WHITE);
  
  for(int i=0; i<numOptions; i++) {
    int y = 15 + i*10;
    if (i == cursor) {
      spr.fillRect(0, y-1, SCREEN_WIDTH, 10, OLED_WHITE);
      spr.setTextColor(OLED_BLACK);
    } else {
      spr.setTextColor(OLED_WHITE);
    }
    spr.setCursor(5, y);
...
void drawAssistantFace() {
  ...
  if (voiceState == VOICE_LISTENING) {
    ...
    spr.fillRoundRect(leftX - pulse, eyeY - pulse, 27 + pulse * 2, 34 + pulse * 2, 9, OLED_WHITE);
    spr.fillRoundRect(rightX - pulse, eyeY - pulse, 27 + pulse * 2, 34 + pulse * 2, 9, OLED_WHITE);
    for (int i = 0; i < 7; i++) {
      int h = 2 + (int)(voiceLevel * (3 + ((i * 3) % 8)));
      h += (int)(fabsf(sinf(phase + i * 0.8f)) * 3.0f);
      spr.drawFastVLine(49 + i * 5, 57 - h / 2, h, OLED_WHITE);
    }
    spr.setCursor(44, 1);
    spr.print("DENGAR");
  } else if (voiceState == VOICE_THINKING) {
    int look = (int)roundf(sinf(now * 0.006f) * 4.0f);
    spr.fillRoundRect(leftX + look, eyeY + 5, 27, 29, 9, OLED_WHITE);
    spr.fillRoundRect(rightX + look, eyeY + 5, 27, 29, 9, OLED_WHITE);
    int dot = (now / 320UL) % 3;
    for (int i = 0; i < 3; i++) {
      if (i <= dot) spr.fillCircle(57 + i * 7, 55, 2, OLED_WHITE);
      else spr.drawCircle(57 + i * 7, 55, 2, OLED_WHITE);
    }
    spr.setCursor(47, 1);
    spr.print("MIKIR");
  } else if (voiceState == VOICE_SPEAKING) {
    int eyeH = 31 + (int)(sinf(now * 0.005f) * 2.0f);
    spr.fillRoundRect(leftX, eyeY, 27, eyeH, 9, OLED_WHITE);
    spr.fillRoundRect(rightX, eyeY, 27, eyeH, 9, OLED_WHITE);
    int mouthH = 3 + (int)(levelSmooth * 10.0f);
    mouthH += (int)(fabsf(sinf(phase)) * 4.0f);
    spr.drawRoundRect(56, 49 - mouthH / 2, 17, mouthH, 5, OLED_WHITE);
    spr.setCursor(47, 1);
    spr.print("JAWAB");
  } else {
    spr.fillRoundRect(leftX, eyeY + 5, 27, 29, 9, OLED_WHITE);
    spr.fillRoundRect(rightX, eyeY + 5, 27, 29, 9, OLED_WHITE);
    spr.drawLine(55, 52, 61, 48, OLED_WHITE);
    spr.drawLine(61, 48, 68, 52, OLED_WHITE);
    spr.setCursor(35, 1);
    spr.print(voiceState == VOICE_ERROR ? "COBA LAGI" : "OWI");
  }
}
...
void drawUI() {
  ...
  if (currentState == STATE_ASSISTANT) {
    ...
    drawPingpong();
    return; // drawPingpong calls spr.pushSprite(0, 0); itself
    ...
    spr.setCursor(30, 0);
    spr.print("DHT22");
    spr.drawLine(0, 9, 128, 9, OLED_WHITE);
    // Temperature with big font
    spr.drawRoundRect(2, 12, 60, 38, 4, OLED_WHITE);
    spr.setCursor(8, 16);
    spr.print("Suhu");
    spr.setTextSize(2);
    spr.setCursor(6, 26);
    ...
    spr.setTextSize(1);
    // Humidity
    spr.drawRoundRect(66, 12, 60, 38, 4, OLED_WHITE);
    spr.setCursor(72, 16);
    spr.print("Humid");
    spr.setTextSize(2);
    spr.setCursor(70, 26);
    if (!isnan(humPct)) {
      spr.print((int)humPct);
      spr.print("%");
    } else spr.print("--%");
    spr.setTextSize(1);
    // Status bar
...
void startAudioStream(String file, String vol) {
  ...
  audioHttp.end();
  audioStream = nullptr;
  String url = "http://212.2.253.247:3001/stream?file=" + file + "&vol=" + vol;
    ...
    case WStype_BIN:
      {
        if (length == 1030 && memcmp(payload, "FRAME:", 6) == 0) {
          if (oledReady) {
            ...
            spr.fillSprite(0);
            drawBitmapScaled(drawFrame, 128, 64, 0, 0, 128, 64);
            pushScaledSprite();
          }
        }
      }
...
void setup() {
  ...
  Wire.begin(); // Standard ESP32 I2C pins (GPIO 21, 22)
  display.begin();
  display.setRotation(1); // Landscape mode 320x240
  spr.setColorDepth(1);
  if (!spr.createSprite(128, 64)) Serial.println("SPRITE FAIL!");
  spr.setBitmapColor(TFT_CYAN, TFT_BLACK);
  spr.fillSprite(0);
  ts.begin();
  ts.setRotation(1);
  oledReady = true;
  ...
  if (!setupI2S()) {
    Serial.println("I2S ERR. Lanjut tanpa I2S.");
  }
